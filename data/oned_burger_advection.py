...
class OneD_Advection_Burgers_MultiParam(object):
    ...
    def __init__(self, pde ,data_root, train=True,
                 ...
                 frame_step=1,
                 shuffle=True, num_param_combinations=-1, 
                 fixed_ic = False, fixed_window = False, 
                 total_size_percent = 1,
                 percent_trajectories = 1,
                 fno_rnn = False, ood = False):
        '''
        ...
        self.length = length
        self.train = train
        self.percent_trajectories = percent_trajectories
        self.h5_paths = glob.glob(f"{self.data_root}/*.hdf5")
        self.h5_paths = self.h5_paths[:int(len(self.h5_paths) * total_size_percent)]
    ...
    def __getitem__(self, index):
        ...
        if int(len(seqs['tensor']) * self.percent_trajectories) == 0:
            random_index = np.random.choice(len(seqs['tensor']))
        else:
            random_index = np.random.choice(int(len(seqs['tensor']) * self.percent_trajectories)) 
