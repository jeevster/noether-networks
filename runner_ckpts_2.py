...
# for seed in [0,1,2]:
#     PATH = f'config_files/1d_advection_multiparam_new/baseline_4k/embedding/run_advection_seed={seed}_param_loss.sh'
#     result = [y for x in os.walk(PATH) for y in glob(os.path.join(x[0], '*.sh'))]
#     for file_path in result:
#         subprocess.run(f'CUDA_VISIBLE_DEVICES={GPU} bash {file_path}',shell=True)
for seed in [0,1,2]:
    PATH = f'config_files/1d_burgers_multiparam_new/baseline_4k/seed={seed}_param/run_data_outer_train=learned_val=learned_param_loss_init_frames_no_pretraining.sh'
    subprocess.run(f'CUDA_VISIBLE_DEVICES={GPU} bash {PATH}',shell=True)

    # PATH = f'config_files/1d_advection_multiparam_new/baseline_4k/seed={seed}_param/run_data_outer_train=learnable_val=learnable_param_loss_init_frames_no_pretraining.sh'
    # subprocess.run(f'CUDA_VISIBLE_DEVICES={GPU} bash {PATH}',shell=True)
