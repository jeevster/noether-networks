...
parser.add_argument('--svg_loss_kl_weight', type=float,
                    default=0.0001, help='weighting factor for KL loss')
...
parser.add_argument('--num_emb_frames', type=int, default=1,
...
parser.add_argument('--save_warmstart_dataset',
                    action='store_true', help='save_warmstart_dataset')
parser.add_argument('--inner_opt_all_model_weights', action='store_true',
                    help='optimize non-CN model weights in inner loop?')
parser.add_argument('--adam_inner_opt', action='store_true',
                    help='use Adam in inner loop?')
...
for trial_num in range(opt.n_trials):
    ...
    for batch_num in tqdm(range(opt.num_val_batch)):
        ...
        with torch.no_grad():
            outer_loss = svg_crit(gen_seq, batch, mus,
                                  logvars, mu_ps, logvar_ps, opt)

        val_outer_loss += outer_loss.detach().cpu().numpy().item()

    all_val_inner_losses.append([sum(x) / (opt.num_val_batch)
                                for x in zip(*val_inner_losses)])
    all_val_svg_losses.append([sum(x) / (opt.num_val_batch)
                              for x in zip(*val_svg_losses)])
    ...
    print(
        f'\tOuter SVG loss:   {np.array(all_val_svg_losses[-1]).mean(axis=(0))}')
    print(
        f'\tInner VAL loss:   {np.array(all_val_inner_losses[-1]).mean(axis=(0))}')
    print(f'\tOuter VAL loss:   {all_outer_losses[-1]}')
    print(
        f'\tOuter SSIM:       {np.array(all_tailor_ssims[-1]).mean(axis=(0,1,-2))}\n\t\tmean: {np.array(all_tailor_ssims[-1]).mean()}')
    print(
        f'\tOuter PSNR:       {np.array(all_tailor_psnrs[-1]).mean(axis=(0,1,-2))}\n\t\tmean: {np.array(all_tailor_psnrs[-1]).mean()}')
    print(
        f'\tOuter MSE:        {np.array(all_tailor_mses[-1]).mean(axis=(0,1,-2))}\n\t\tmean: {np.array(all_tailor_mses[-1]).mean()}')
