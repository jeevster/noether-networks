...
def desired_func(config):
        ...
        input_params = [
                ...
                '--log_dir','/data/divyam123/results_noether_summer/advection/pretrained_embedding_true_conditioning/data_true(outer)',
                '--channels','1',
                '--random_weights',
            ...
            if opt.random_weights:
                ...
                if opt.dataset in  set(['1d_burgers_multiparam','1d_advection_multiparam','1d_diffusion_reaction_multiparam']):
                    frame_predictor = CNFNO1d(n_modes_height=opt.fno_modes, 
                                            ...
                                            in_channels=(opt.channels + 1)*opt.n_past, 
                                            out_channels=opt.channels, 
                                            n_layers=opt.fno_layers,
                                            norm = 'batch_norm' if opt.use_cn else None, # added just now
                                            #### CN Layer Parameters ####
                                            batch_size = opt.batch_size,
                                            use_cn = opt.use_cn).to(torch.float64)
                ...
                # frame_predictor = lstm_models.lstm(opt.g_dim+opt.z_dim+opt.a_dim, opt.g_dim, opt.rnn_size, opt.predictor_rnn_layers, opt.batch_size)
                    ...
                    frame_predictor = FNO(n_modes=(opt.fno_modes, opt.fno_modes), hidden_channels=opt.fno_width,
                                        in_channels=opt.channels*opt.n_past, out_channels=opt.channels, n_layers=opt.fno_layers).to(torch.float64)
                    ...
                    for batch_num in tqdm(range(opt.num_val_batch)):
                        ...
                        for batch_step in range(opt.num_jump_steps + 1):
                            ...
                            gen_seq, mus, logvars, mu_ps, logvar_ps = tailor_many_steps(
                                ...
                                svg_model, batch, true_pde_embedding, params, #if opt.use_true_params_val else pred_params, 
                                opt=opt, track_higher_grads=False,
    ...
    tuner = tune.Tuner(
        ...
        tune_config=tune.TuneConfig(
                ...
                run_config = RunConfig(
                                    ...
                                    name = "advection_conditioning_data_true(outer)",
                                )
    )
    results = tuner.fit()
