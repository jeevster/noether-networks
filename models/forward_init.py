...
def inner_crit(fmodel, gen_seq, true_params, mode='mse', num_emb_frames=1, compare_to='prev', setting='train', opt=None, emb_mode = 'emb', learnable_model = None):
    ...
    if num_emb_frames == 1:
        ...
        # embs = [fmodel(frame, true_params = true_params, mode=emb_mode, return_params = True)[0] for frame in stacked_gen_seq]
        val_nu_loss = 0
...
def predict_many_steps(func_model, gt_seq, true_params, opt, mode='eval', prior_epses=[], posterior_epses=[], learnable_model = None):
            ...
            for inner_step in range(opt.num_inner_steps):
                ...
                with torch.no_grad():
                        true_tailor_loss, true_param_loss = inner_crit(fmodel, gen_seq, params, mode=inner_crit_mode,
                                            ...
                                            setting=mode, opt=opt, emb_mode = 'true_emb')
