...
def predict_many_steps(func_model, gt_seq, true_params, opt, mode='eval', prior_epses=[], posterior_epses=[], learnable_model = False, phi_hat = None,rank = None):
...
def tailor_many_steps(svg_model, x, true_pde_embedding, params, opt, track_higher_grads=True, mode='eval',learnable_model = None, **kwargs):
    ...
    # TODO: uncomment
    replace_cn_layers(svg_model.module.encoder)
    replace_cn_layers(svg_model.module.decoder)
    replace_cn_layers(svg_model.module.frame_predictor)
    ...
    # _cn_beta = list(filter(lambda p: 'beta' in p[0], svg_model.module.decoder.named_parameters()))
    # print(f'CN layer gamma: {_cn_beta[1]}')

    cn_module_params = list(svg_model.module.decoder.named_parameters())
    if not 'only_cn_decoder' in kwargs or not kwargs['only_cn_decoder']:
        cn_module_params += list(svg_model.module.encoder.named_parameters())

    cn_params = [p[1] for p in cn_module_params if ('gamma' in p[0] or 'beta' in p[0])]

    if 'Basic' in type(svg_model).__name__:
        cn_params = list(svg_model.module.encoder.parameters()) + list(svg_model.module.decoder.parameters())

    # elif opt.inner_opt_all_model_weights:
        # TODO: try with ALL modules, not just enc and dec
        # cn_params = list(svg_model.module.encoder.parameters()) + list(svg_model.module.decoder.parameters()) \
            # + list(svg_model.module.prior.parameters()) + list(svg_model.module.posterior.parameters()) + \
                # list(svg_model.module.frame_predictor.parameters())

    frame_predictor_params =  list(svg_model.module.frame_predictor.named_parameters())
    if opt.use_cn == True and opt.inner_opt_all_model_weights == False:
        ...
        cn_params += list(svg_model.module.frame_predictor.parameters())
    ...
    cache = cache_cn_modules(svg_model.module)
    try:
        if mode == 'train':
            ...
            for inner_step in range(num_inner_steps):
                ...
                if opt.learn_inner_lr:
                    ...
                    loss *= torch.exp(list(filter(lambda x: x.size() == torch.Size([]),
                                                  [param for param in svg_model.module.parameters()]))[0])
