...
import pdb
from models.forward_parallel import predict_many_steps, tailor_many_steps
...
parser.add_argument('--restore_path', default='',
                    help='directory to reload checkpoints from')
parser.add_argument('--restore_epoch', type=int, default=0,
                    help='epoch to reload training from')
...
# mp.spawn(main, args=(opt ,world_size), nprocs=world_size)
...
def main(rank: int, opt, world_size):
    ...
    def save_checkpoint(model, outer_opt, log_dir, best_outer = False, best_inner = False, best_svg = False):
        chosen_file = ''
        ...
        if rank == 0:
            print('%s/%s' % (log_dir, chosen_file))
            torch.save({'model_state': model.state_dict()},'%s/%s' % (log_dir, chosen_file))
            torch.save({'outer_opt':outer_opt.state_dict()},'%s/%s' % (log_dir, 'outer_opt.pt'))

    def restore_checkpoint(model, log_dir, device, best_outer = False, best_inner = False, best_svg = False):
        # if len(os.listdir(log_dir)) >= 2:
        # log_dir = '/global/cfs/projectdirs/m4319/divyam123/2d_react_diff_multiparam/TRIAL/layer_norm_steps=5/data_outer_true_inner_num_steps=5_layer_norm_opt=adam_weights=cn/Sat-Jan-27-15.23.16-2024_past=2_future=2_tailor=PDE_Const_1'
        # chosen_file = ''
        print("reloading mdoel")
        if best_outer == False and best_inner == False and best_svg == False:
            chosen_file = 'ckpt_model.pt'
        elif best_outer == True and best_inner == False and best_svg == False:
            chosen_file = 'best_outer_val_ckpt_model.pt'
        ...
        checkpoint_path = os.path.join(opt.restore_path, 'ckpt_model.pt')
        checkpoint = torch.load(checkpoint_path, map_location= device)
        model.module.load_state_dict(checkpoint['model_state'])
        
        # checkpoint_path = os.path.join(log_dir, 'outer_opt.pt')
        # checkpoint = torch.load(checkpoint_path, map_location= device)
        # outer_opt.load_state_dict(checkpoint['outer_opt'])
    ...
    for trial_num in range(opt.num_trials):
        ...
        if opt.random_weights:
            ...
            # frame_predictor = lstm_models.lstm(opt.g_dim+opt.z_dim+opt.a_dim, opt.g_dim, opt.rnn_size, opt.predictor_rnn_layers, opt.batch_size)
                ...
                frame_predictor = CNFNO(n_modes=(opt.fno_modes, opt.fno_modes), hidden_channels=opt.fno_width,
                                    in_channels=opt.channels*opt.n_past, out_channels=opt.channels, n_layers=opt.fno_layers,val_batch_size = opt.val_batch_size,
                                        train_batch_size = opt.train_batch_size,
                                        use_cn = opt.use_cn,norm = opt.norm if opt.norm != '' else None,)
            ...
            svg_model = SVGModel(encoder, frame_predictor,
                                decoder, prior, posterior, outer_embedding if opt.use_embedding else embedding, true_pde_embedding).to(f'cuda:{rank}')
            svg_model.apply(lambda t: t.cuda(rank))
            
            svg_model = DDP(svg_model, device_ids = [rank])
        ...
        replace_cn_layers(svg_model.module.encoder)#, batch_size=opt.batch_size)
        replace_cn_layers(svg_model.module.decoder)#, batch_size=opt.batch_size)
        replace_cn_layers(svg_model.module.frame_predictor)#, batch_size=opt.batch_size)
        svg_model.module.frame_predictor.batch_size = opt.batch_size
        svg_model.module.posterior.batch_size = opt.batch_size
        svg_model.module.prior.batch_size = opt.batch_size
        ...
        if opt.warmstart_emb_path != '' and opt.tailor and opt.emb_type != "pde_const_emb":
            emb_ckpt = torch.load(opt.warmstart_emb_path)
            svg_model.module.emb.load_state_dict(emb_ckpt['model_state'])

        # svg_model.module.apply(lambda t: t.cuda())
        # print('Eval summary')
        # summary(svg_model.module.frame_predictor, input_size=(1, opt.n_past*opt.channels, opt.image_width,
        ...
        # print('Emb summary')
        # #summary(svg_model.module.emb, input_size=(opt.n_past, opt.channels, opt.image_width, opt.image_width), device=torch.device("cuda"))
        # summary(svg_model.module.emb, input_size=(1, opt.num_emb_frames * opt.channels, opt.image_width, opt.image_width), dtypes=[torch.float64], device=torch.device("cuda"))
        # For comparing later
        old_state_dict = copy.deepcopy(svg_model.module.state_dict())
        if opt.baseline:
            # only useful for tailoring or meta-tailoring from a checkpoint
            baseline_svg_model = copy.deepcopy(svg_model)
            for param in baseline_svg_model.module.parameters():
                param.requires_grad = False
            baseline_svg_model.module.cuda()
            baseline_svg_model.module.eval()

        # TODO NC: I'm pretty sure none of this needs to be changed since we're using identity now.

        # Init 'inner' optimizer
        emb_params = [p[1] for p in svg_model.module.emb.named_parameters() if not ('gamma' in p[0] or 'beta' in p[0])]

        # Dont do this
        if opt.encoder_emb and not opt.optimize_emb_enc_params:
            emb_params = list(svg_model.module.emb.linear.parameters())
        ...
        if opt.outer_opt_model_weights:
            ...
            # encoder and decoder
            non_cn_params = [p[1] for p in list(svg_model.module.encoder.named_parameters()) + list(svg_model.module.decoder.named_parameters())
                                                                                    if not ('gamma' in p[0] or 'beta' in p[0])]
            outer_params = non_cn_params + emb_params + \
                list(svg_model.module.prior.parameters()) + \
                list(svg_model.module.posterior.parameters()) + \
                list(svg_model.module.frame_predictor.parameters())
        ...
        if opt.learn_inner_lr:
            svg_model.module.inner_lr_scale = torch.nn.Parameter(torch.tensor(0.))
            outer_params.append(svg_model.module.inner_lr_scale)
        ...
        if opt.reload_latest:
            ...
            checkpoint_path = os.path.join(opt.restore_path, 'outer_opt.pt')
            checkpoint = torch.load(checkpoint_path, map_location= device)
            outer_opt.load_state_dict(checkpoint['outer_opt'])    
        ...
        for p in svg_model.module.named_parameters():
            print(p[1].dtype)
            # assert p[
        ...
        for epoch in range(start_epoch, opt.n_epochs):
            ...
            if epoch % opt.num_epochs_per_val == 0:
                    ...
                    for batch_step in range(opt.num_jump_steps + 1):
                        ...
                        if opt.conditioning:
                            if opt.use_true_params_val:
                                ...
                                if opt.dataset == '1d_diffusion_reaction_multiparam':
                                    phi_hat = torch.tile(torch.stack(tuple(pred_params[0], params[1]), dim = 1).unsqueeze(-1).unsqueeze(-1), dims = (1,1,1024,1))
                        ...
                        gen_seq, mus, logvars, mu_ps, logvar_ps = tailor_many_steps(
                            ...
                            svg_model, [data.to(rank) for data in batch], true_pde_embedding, tuple([param.to(rank) for param in params]), opt=opt, track_higher_grads=False,
                ...
                if opt.tailor:
                    for step, value in enumerate(val_inner_losses[-1]):
                        ...
                        writer.add_scalar(
                            f'Log Inner Loss/val/{step} Step', val_abs_inner_losses[-1][step], (epoch + 1))
                ...
                try:
                    for step, value in enumerate(val_true_inner_losses[-1]):
                        ...
                        writer.add_scalar(
                            f'True Log Inner Loss/val/{step} Step', val_true_abs_inner_losses[-1][step], (epoch + 1))
                        writer.add_scalar(
                            f'True Inner Param Loss/val/{step} Step', val_true_param_losses[-1][step], (epoch + 1))
                    writer.add_scalar(
                        f'Data Loss Gain/val', val_data_gain[-1], (epoch + 1))
                    writer.add_scalar(
                            f'True Log Inner Loss/val/Final', val_true_abs_inner_losses[-1][-1], (epoch + 1))
                    writer.add_scalar(
            ...
            for batch_num in tqdm(range(opt.num_train_batch)):
                ...
                for batch_step in range(opt.num_jump_steps + 1):
                    ...
                    gen_seq, mus, logvars, mu_ps, logvar_ps = tailor_many_steps(
                                                                svg_model, [data.to(rank) for data in batch], true_pde_embedding, [param.to(rank) for param in params], #if opt.use_true_params_train else pred_params,
                    ...
                    if opt.use_embedding:
                        ...
                        # # print("pde_value",pde_value)
                        # embedding_loss = (embedding_true_pde_value ** 2).mean()
                    ...
                    if opt.train_num_inner_steps > 0:
                        ...
                        param_grads.append(
                            [-1. if p.grad is None else torch.norm(p.grad).item() for p in svg_model.module.parameters()])
                        grad_norm = nn.utils.clip_grad_norm_(
                            svg_model.module.emb.parameters(), 1000)
                        grad_norm_sum += grad_norm.item()
                        emb_p = [torch.norm(p.detach())
                                for p in svg_model.module.emb.parameters()]
                        if len(emb_p) > 0:
                ...
                outer_opt.step()
                svg_model.module.zero_grad(set_to_none=True)
                if opt.use_embedding:
            ...
            if opt.tailor:
                for step, value in enumerate(inner_losses[-1]):
                    ...
                    writer.add_scalar(
                        f'Log Inner Loss/train/{step} Step', abs_inner_losses[-1][step], (epoch + 1))
                    writer.add_scalar(
            ...
            try:
                for step, value in enumerate(true_inner_losses[-1]):
                    ...
                    writer.add_scalar(
                        f'True Log Inner Loss/train/{step} Step', true_abs_inner_losses[-1][step], (epoch + 1))
                    writer.add_scalar(
                        f'True Inner Param Loss/train/{step} Step', true_param_losses[-1][step], (epoch + 1))
                writer.add_scalar(
                    f'True Log Inner Loss/train/Final', true_abs_inner_losses[-1][-1], (epoch + 1))
            ...
            if opt.tailor:
                ...
                writer.add_scalar('Embedding/param norm', emb_norms[-1],
                                (epoch + 1))
            if opt.learn_inner_lr:
                writer.add_scalar('Embedding/inner LR scale factor', svg_model.module.inner_lr_scale,
                                (epoch + 1))
                all_inner_lr_scales.append(
                    svg_model.module.inner_lr_scale.detach().cpu().item())
            ...
            if opt.save_checkpoint:
                ...
                save_checkpoint(svg_model,outer_opt,save_dir)
...
if __name__ == '__main__':
    ...
    mp.spawn(main, args=(opt ,world_size), nprocs=world_size, join = True)
